...
function bcSyncSortLink(string $col, string $current, string $dir, string $label): string {
    $newDir = ($current === $col && $dir === 'DESC') ? 'ASC' : 'DESC';
    $icon   = $current === $col ? ($dir === 'ASC' ? ' ↑' : ' ↓') : '';
    $params = ['sort' => $col, 'dir' => $newDir];
    return '<a href="/becloud/sync-logs?' . http_build_query($params) . '" class="text-white text-decoration-none">'
         . htmlspecialchars($label) . $icon . '</a>';
}
?>

<div class="d-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0">Historique sync Be-Cloud</h1>
    <a href="/becloud/licenses" class="btn btn-outline-secondary">
        <i class="bi bi-arrow-left me-1"></i>Retour aux abonnements
    </a>
</div>
...
<div class="table-responsive">
    <table class="table table-hover table-sm align-middle">
        <thead class="table-dark">
            <tr>
                <th><?= bcSyncSortLink('started', $sortBy, $sortDir, 'Démarrage') ?></th>
                <th><?= bcSyncSortLink('finished', $sortBy, $sortDir, 'Fin') ?></th>
                <th>Durée</th>
                <th><?= bcSyncSortLink('connection', $sortBy, $sortDir, 'Connexion') ?></th>
                <th class="text-center"><?= bcSyncSortLink('trigger', $sortBy, $sortDir, 'Déclencheur') ?></th>
                <th class="text-center"><?= bcSyncSortLink('status', $sortBy, $sortDir, 'État') ?></th>
                <th class="text-center"><?= bcSyncSortLink('fetched', $sortBy, $sortDir, 'Récupérés') ?></th>
                <th class="text-center"><?= bcSyncSortLink('created', $sortBy, $sortDir, 'Créés') ?></th>
                <th class="text-center"><?= bcSyncSortLink('updated', $sortBy, $sortDir, 'Mis à jour') ?></th>
