...
function ikSyncSortLink(string $col, string $current, string $dir, string $label): string {
    $newDir = ($current === $col && $dir === 'DESC') ? 'ASC' : 'DESC';
    $icon   = $current === $col ? ($dir === 'ASC' ? ' ↑' : ' ↓') : '';
    $params = ['sort' => $col, 'dir' => $newDir];
    return '<a href="/infomaniak/sync-logs?' . http_build_query($params) . '" class="text-white text-decoration-none">'
         . htmlspecialchars($label) . $icon . '</a>';
}
?>

<div class="d-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0">Historique sync Infomaniak</h1>
    <a href="/infomaniak/licenses" class="btn btn-outline-secondary">
...
<div class="table-responsive">
    <table class="table table-hover table-sm align-middle">
        <thead class="table-dark">
            <tr>
                <th><?= ikSyncSortLink('started', $sortBy, $sortDir, 'Démarrage') ?></th>
                <th><?= ikSyncSortLink('finished', $sortBy, $sortDir, 'Fin') ?></th>
                <th>Durée</th>
                <th><?= ikSyncSortLink('connection', $sortBy, $sortDir, 'Connexion') ?></th>
                <th class="text-center"><?= ikSyncSortLink('trigger', $sortBy, $sortDir, 'Déclencheur') ?></th>
                <th class="text-center"><?= ikSyncSortLink('status', $sortBy, $sortDir, 'État') ?></th>
                <th class="text-center"><?= ikSyncSortLink('fetched', $sortBy, $sortDir, 'Récupérés') ?></th>
                <th class="text-center"><?= ikSyncSortLink('created', $sortBy, $sortDir, 'Créés') ?></th>
                <th class="text-center"><?= ikSyncSortLink('updated', $sortBy, $sortDir, 'Mis à jour') ?></th>
