...
function nSyncSortLink(string $col, string $current, string $dir, string $label): string {
    $newDir = ($current === $col && $dir === 'DESC') ? 'ASC' : 'DESC';
    $icon   = $current === $col ? ($dir === 'ASC' ? ' ↑' : ' ↓') : '';
    $params = ['sort' => $col, 'dir' => $newDir];
    return '<a href="/ninjaone/sync-logs?' . http_build_query($params) . '" class="text-white text-decoration-none">'
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0">
        <i class="bi bi-hdd-network text-warning me-2"></i>Historique sync NinjaOne
    </h1>
    <a href="/ninjaone/licenses" class="btn btn-outline-secondary">
        <i class="bi bi-arrow-left me-1"></i>Retour aux équipements
    </a>
</div>
...
<div class="table-responsive">
    <table class="table table-hover table-sm align-middle">
        <thead class="table-dark">
            <tr>
                <th><?= nSyncSortLink('started', $sortBy, $sortDir, 'Démarrage') ?></th>
                <th><?= nSyncSortLink('finished', $sortBy, $sortDir, 'Fin') ?></th>
                <th>Durée</th>
                <th><?= nSyncSortLink('connection', $sortBy, $sortDir, 'Connexion') ?></th>
                <th class="text-center"><?= nSyncSortLink('trigger', $sortBy, $sortDir, 'Déclencheur') ?></th>
                <th class="text-center"><?= nSyncSortLink('status', $sortBy, $sortDir, 'État') ?></th>
                <th class="text-center"><?= nSyncSortLink('fetched', $sortBy, $sortDir, 'Récupérés') ?></th>
                <th class="text-center"><?= nSyncSortLink('created', $sortBy, $sortDir, 'Créés') ?></th>
                <th class="text-center"><?= nSyncSortLink('updated', $sortBy, $sortDir, 'Mis à jour') ?></th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="small"><?= htmlspecialchars($log['connection_name'] ?? '—') ?></td>
